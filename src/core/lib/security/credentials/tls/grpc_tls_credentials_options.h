 ...
 public:
  ...
  int Schedule(grpc_tls_credential_reload_arg* arg) const {
    if (schedule_ == nullptr) {
      ...
      if (arg != nullptr) {
        ...
        arg->error_details->set_err_details("schedule API in credential reload config is nullptr");
      }
      return 1;
    }
    if (arg != nullptr) {
  ...
  void Cancel(grpc_tls_credential_reload_arg* arg) const {
    if (cancel_ == nullptr) {
      ...
      if (arg != nullptr) {
        arg->status = GRPC_SSL_CERTIFICATE_CONFIG_RELOAD_FAIL;
        arg->error_details->set_err_details("cancel API in credential reload config is nullptr");
      }
      return;
    }
    if (arg != nullptr) {
