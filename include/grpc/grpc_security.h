...
/** A struct containing all information necessary to schedule/cancel a
    credential reload request. To help understand each fields, we mark fields
    with some labels and here are the explanations of each labels:
    -------------------------label explanation----------------------------------
    ...
    [wrap-lang]: fields that need to retrieve the particular implementation in
    wrap language
    [async-only]: fields that only used when in asynchronous mode. Without this
    label, a field need to be specified both in synchronous and asynchronous
    mode.
    ...
    - key_materials_config[user-managed][system-provided]: an in/output
      ...
      key_materials_config should not be modified. If set to
      |GRPC_SSL_CERTIFICATE_CONFIG_RELOAD_NEW|, key_materials_config object can
      be updated if new key materials is available.
      ...
      1. on client side, if retry mechanism happens, one security_connector can
      map to multiple connections
    ...
    - status[user-managed]: a per-connection parameter to indicate the finish
      ...
      perform credential reloading;
      If set to |GRPC_SSL_CERTIFICATE_CONFIG_RELOAD_FAIL|, users also need to
      set |error_details| to pass the detailed error message to gRPC stack.
...
/** A struct containing all information necessary to schedule/cancel a server
    authorization check request.To help understand each fields, we mark fields
    with some labels and here are the explanations of each labels:
    -------------------------label explanation----------------------------------
    [user-managed]: fields that are managed directly by the end users
    [user-observed]: fields that are exposed but not modifiable by users
    [system-provided]: fields that needed to retrieve implementations in C core
    and  would be provided by the gRPC stack
    [wrap-lang]: fields that need to retrieve the particular implementation in
    wrap language.
    ...
    - peer_cert[user-observed]: a per connection parameter representing a
      complete certificate chain of the peer identity, including both signing
